...
public class ConfigureChatGptDialog extends DialogWrapper {
    ...
    protected void init() {
        ...
        serversList.setEmptyText(MultilingualUtil.getKey("No_servers"));
        ...
        setTitle(MultilingualUtil.getKey("Configure_Servers"));
        super.init();
    }
    ...
    private boolean validateServers() {
        ...
        for (ChatGPTCopilotServer server : servers) {
            if (StringUtils.isBlank(server.getName())) {
                Messages.showErrorDialog(MultilingualUtil.getKey("Server_url_is_required"), MultilingualUtil.getKey("Error"));
                return false;
            }
            if (AuthType.USER_PASS.equals(server.getType())) {
                if (StringUtils.isBlank(server.getUsername())) {
                    Messages.showErrorDialog(MultilingualUtil.getKey("Username_is_required"), MultilingualUtil.getKey("Error"));
                    return false;
                }
                if (StringUtils.isBlank(server.getPassword())) {
                    Messages.showErrorDialog(MultilingualUtil.getKey("Password_is_required"), MultilingualUtil.getKey("Error"));
                    return false;
                }
            } else if (AuthType.API_TOKEN.equals(server.getType())) {
                if (StringUtils.isBlank(server.getApiToken())) {
                    Messages.showErrorDialog(MultilingualUtil.getKey("Personal_Access_Token_is_required"), MultilingualUtil.getKey("Error"));
                    return false;
                }
            }

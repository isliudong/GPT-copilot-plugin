...
import static com.ld.chatgptcopilot.render.InlineDocumentationImplKtKt.inlineDocumentationItems;
...
public class DocRenderPassFactory implements TextEditorHighlightingPassFactoryRegistrar, TextEditorHighlightingPassFactory, DumbAware {
    ...
    public static Items calculateItemsToRender(@NotNull Editor editor, @NotNull PsiFile psiFile) {
        ...
        for (InlineDocumentation documentation : inlineDocumentationItems(psiFile)) {
            ...
            if (DocRenderItem.isValidRange(editor, range)) {
                String textToRender = enabled ? getText(documentation) : null;
    ...
    static @NotNull @Nls String getText(@Nullable InlineDocumentation documentation) {
        StringBuilder stringBuilder = new StringBuilder();
        if (documentation != null) {
            PsiElement[] descriptionElements = ((PsiDocCommentImpl) (((PsiCommentInlineDocumentation) documentation).getComment())).getDescriptionElements();
            for (PsiElement descriptionElement : descriptionElements) {
                if (descriptionElement instanceof PsiDocToken) {
                    if (((PsiDocToken) descriptionElement).getTokenType().equals(DOC_COMMENT_DATA)) {
                        stringBuilder.append(descriptionElement.getText());
                    }
                }
            }
        }
